...
setwd('/Users/akg6325/Dropbox/Github/PhDProjects/Fiji_StabilityConnectivitySimulation')
...
load("Final_Simulations/wcsonly_current_fiji_herbivoredata.RData")
...
#Make j run through a dataframe of different g, a, d_median, initial condition scenarios
#make a couple of different dataframes and then stick them together
#a_vals <- c(0.05,0.07,0.15,0.2) #only need the new values
#d_meds <- c(0.12,0.16,0.36,0.48) #only need the new values
initconds <- c(0.01,0.98)
#varying_a <- data.frame(a = rep(a_vals, each = length(scalingfactors)), d = 0.24, g = rep(scalingfactors,length(a_vals)), C_init = NA, M_init = NA, vary = NA)
#varying_d <- data.frame(a = 0.1, d = rep(d_meds, each = length(scalingfactors)), g = rep(scalingfactors,length(d_meds)), C_init = NA, M_init = NA, vary = NA)
varying_initcond <- data.frame(a = 0.1, d = 0.24, g = rep(scalingfactors,length(initconds)), C_init = rep(initconds, each = length(scalingfactors)), M_init = rep(c(0.98,0.01), each = length(scalingfactors)), vary = "n")
#paramsies <- rbind(varying_a, varying_d, varying_initcond)
paramsies <- rbind(varying_initcond)

for(q in 1:length(altsed)){ #sedimentation scenarios 
  for(k in 1:length(buffsize)){ #tabu scenarios
    for(j in 1:dim(paramsies)[1]){ #1:dim(paramsies)[1] #sensitivity analysis, grazing level and initial condition scenarios
      ...
      if(k == 1){ #no buffer #OLD: 2km buffer
        load("Final_Simulations/benthicfish_masterdataset_sedimentlvls_5.6.2023.RData") #no buffer, benthicfish_masterdataset_sed
        benthicfish_masterdataset <- benthicfish_masterdataset_sed
        g_val <- benthicfish_masterdataset$comb_grazinglevel*paramsies$g[j] 
      }
      if(k == 2){ #qoli qoli det'd buffer
        ...
        benthicfish_masterdataset <- benthicfish_masterdataset_sed
        g_val <- benthicfish_masterdataset$new_grazinglevel*paramsies$g[j] 
      }
      ...
      a_val <- paramsies$a[j] #rate macroalgae overgrows coral
      d_val <- (paramsies$d[j]/0.24)*benthicfish_masterdataset$sediment_altalt*(1-(0.1*(q_i-1))) #if q_i = 3.5 -> multiplying by 0.75
      ...
      parameters <- list(a <- a_val, d <- d_val, g <- g_val, r <- 6500, gamma <- 1900, p_c <- pc_vec, p_m <- pm_vec, l <- length(sitevector))
      #giving M1 and M2, C1 and C2, T1 and T2 starting conditions
      if(is.na(paramsies$M_init[j])){
      ...
      if(!(is.na(paramsies$M_init[j])) & paramsies$vary[j] == "n"){
        state <- c(Ma = rep(paramsies$M_init[j], length(sitevector)), Co = rep(paramsies$C_init[j], length(sitevector)), Tu = rep((1 - paramsies$M_init[j] -  paramsies$C_init[j]), length(sitevector)))
      }
      if(!(is.na(paramsies$M_init[j])) & paramsies$vary[j] == "y"){ 
        
        malg_placeholder <- (paramsies$M_init[j]/median(benthicfish_masterdataset$malg_cover))*(benthicfish_masterdataset$malg_cover)
        malg_placeholder[which(malg_placeholder>0.99)] <- 0.98
        coral_placeholder <- (paramsies$C_init[j]/median(benthicfish_masterdataset$coral_cover))*(benthicfish_masterdataset$coral_cover)
        coral_placeholder[which(coral_placeholder>0.99)] <- 0.98
        if(paramsies$M_init[j] == 0.8){ #this only works in the setting where this value is either 0.8 or 0.1
          coral_placeholder[which(malg_placeholder+coral_placeholder > 1)] <- 0.01 #after this is done, none of the summed values are > 1
        }
        if(paramsies$C_init[j] == 0.8){ #this only works in the setting where this value is either 0.8 or 0.1
          malg_placeholder[which(malg_placeholder+coral_placeholder > 1)] <- 0.01 #after this is done, none of the summed values are > 1
        }
        
        state <- c(Ma = malg_placeholder, Co = coral_placeholder, Tu = (1 - (malg_placeholder+coral_placeholder)))
        coral_placeholder <- malg_placeholder <- turf_placeholder <- NA
      }
      ...
      saveRDS(datalist, file = paste0("Final_Simulations/AddingMoreSensitivityAnalyses_MoreInitConds/SI_Tabu",buffsize[k],"_fullsimrun_allherbs_dval_",altsed[q],"_a",paramsies$a[j],"d",paramsies$d[j],"_scalingfactor",j,"_",inity,"Cinit_5.15.2025.rds")) 
      ...
      BaseRun_newrgamma_allherbivores_InitialCoralCover <-  
        ...
        xlab("Longitude") + ylab("Latitude")+ ggtitle(paste("Initial Coral Cover, Scaling Factor",j," d = ", altsed[q], "buffer size = ", buffsize[k],"a =",paramsies$a[j],"d_med = ",paramsies$d[j],"Cinit = ",inity))+
      ...
      ggsave(BaseRun_newrgamma_allherbivores_InitialCoralCover, filename = paste0("Final_Simulations/AddingMoreSensitivityAnalyses_MoreInitConds/ExtraGraphs/SI_Tabu",buffsize[k],"_dval_",altsed[q],"_a",paramsies$a[j],"d",paramsies$d[j],"_",inity,"Cinit_allherbivores_InitialCoralCover_ScalingFactor",j,"_5.15.2025.png"), bg = "transparent", height = 10, width = 10) 
      
      #final coral cover
      BaseRun_newrgamma_FinalCoralCover <-  
        ggplot(aes(x = long, y = lat), data = worldmap) + 
        geom_polygon(aes(group = group), fill="#f9f9f9", colour = "grey65") +
        xlab("Longitude") + ylab("Latitude")+ ggtitle(paste("Final Coral Cover, Scaling Factor",j," d = ", altsed[q], "buffer size = ", buffsize[k],"a =",paramsies$a[j],"d_med = ",paramsies$d[j],"Cinit = ",inity))+
        ...
        scale_x_continuous(limits = c(180 - 3, 180 + 1), #c(180 - 3, 180 + 2.5) #c(180 - 3, 180)
                           ...
                           labels = c(160, 170, "180/-180", -170)) +
        coord_equal() +  theme_bw()
      ggsave(BaseRun_newrgamma_FinalCoralCover, filename = paste0("Final_Simulations/AddingMoreSensitivityAnalyses_MoreInitConds/ExtraGraphs/SI_Tabu",buffsize[k],"_dval_",altsed[q],"_a",paramsies$a[j],"d",paramsies$d[j],"_",inity,"Cinit_FinalCoralCover_ScalingFactor",j,"_5.15.2025.png"), bg = "transparent", height = 10, width = 10)
      
      #initial malg cover
      BaseRun_newrgamma_InitialMalgCover <-  
        ggplot(aes(x = long, y = lat), data = worldmap) + 
        geom_polygon(aes(group = group), fill="#f9f9f9", colour = "grey65") +
        xlab("Longitude") + ylab("Latitude")+ ggtitle(paste("Initial Malg Cover, Scaling Factor",j," d = ", altsed[q], "buffer size = ", buffsize[k],"a =",paramsies$a[j],"d_med = ",paramsies$d[j],"Cinit = ",inity))+
        ...
        scale_x_continuous(limits = c(180 - 3, 180 + 1), #c(180 - 3, 180 + 2.5) #c(180 - 3, 180)
                           ...
                           labels = c(160, 170, "180/-180", -170)) +
        coord_equal() +  theme_bw()
      ggsave(BaseRun_newrgamma_InitialMalgCover, filename = paste0("Final_Simulations/AddingMoreSensitivityAnalyses_MoreInitConds/ExtraGraphs/SI_Tabu",buffsize[k],"_dval_",altsed[q],"_a",paramsies$a[j],"d",paramsies$d[j],"_",inity,"Cinit_InitialMalgCover_ScalingFactor",j,"_5.15.2025.png"), bg = "transparent", height = 10, width = 10)
      
      #final malg cover
      BaseRun_newrgamma_FinalMalgCover <-  
        ggplot(aes(x = long, y = lat), data = worldmap) + 
        geom_polygon(aes(group = group), fill="#f9f9f9", colour = "grey65") +
        xlab("Longitude") + ylab("Latitude")+ ggtitle(paste("Final Malg Cover, Scaling Factor",j," d = ", altsed[q], "buffer size = ", buffsize[k],"a =",paramsies$a[j],"d_med = ",paramsies$d[j],"Cinit = ",inity))+
        ...
        scale_x_continuous(limits = c(180 - 3, 180 + 1), #c(180 - 3, 180 + 2.5) #c(180 - 3, 180)
                           ...
                           labels = c(160, 170, "180/-180", -170)) +
        coord_equal() +  theme_bw()
      ggsave(BaseRun_newrgamma_FinalMalgCover, filename = paste0("Final_Simulations/AddingMoreSensitivityAnalyses_MoreInitConds/ExtraGraphs/SI_Tabu",buffsize[k],"_dval_",altsed[q],"_a",paramsies$a[j],"d",paramsies$d[j],"_",inity,"Cinit_allherbivores_FinalMalgCover_ScalingFactor",j,"_5.15.2025.png"), bg = "transparent", height = 10, width = 10)
      ...
      Rounded_BaseRun_newrgamma_allherbivores_InitialCoralCover <-  
        ggplot(aes(x = long, y = lat), data = worldmap) + 
        geom_polygon(aes(group = group), fill="#f9f9f9", colour = "grey65") +
        xlab("Longitude") + ylab("Latitude")+ ggtitle(paste("Rounded Initial Coral Cover, Scaling Factor",j," d = ", altsed[q], "buffer size = ", buffsize[k],"a =",paramsies$a[j],"d_med = ",paramsies$d[j],"Cinit = ",inity))+
        ...
        scale_x_continuous(limits = c(180 - 3, 180 + 1), #c(180 - 3, 180 + 2.5) #c(180 - 3, 180)
                           ...
                           labels = c(160, 170, "180/-180", -170)) +
        coord_equal() +  theme_bw()
      ggsave(Rounded_BaseRun_newrgamma_allherbivores_InitialCoralCover, filename = paste0("Final_Simulations/AddingMoreSensitivityAnalyses_MoreInitConds/ExtraGraphs/Rounded_SI_Tabu",buffsize[k],"_dval_",altsed[q],"_a",paramsies$a[j],"d",paramsies$d[j],"_",inity,"Cinit_allherbivores_InitialCoralCover_ScalingFactor",j,"_5.15.2025.png"), bg = "transparent", height = 10, width = 10)
      ...
      Rounded_BaseRun_newrgamma_allherbivores_FinalCoralCover <-  
        ggplot(aes(x = long, y = lat), data = worldmap) + 
        geom_polygon(aes(group = group), fill="#f9f9f9", colour = "grey65") +
        xlab("Longitude") + ylab("Latitude")+ ggtitle(paste("Rounded Final Coral Cover, Scaling Factor",j," d = ", altsed[q], "buffer size = ", buffsize[k],"a =",paramsies$a[j],"d_med = ",paramsies$d[j],"Cinit = ",inity))+
        ...
        scale_x_continuous(limits = c(180 - 3, 180 + 1), #c(180 - 3, 180 + 2.5) #c(180 - 3, 180)
                           ...
                           labels = c(160, 170, "180/-180", -170)) +
        coord_equal() +  theme_bw()
      ggsave(Rounded_BaseRun_newrgamma_allherbivores_FinalCoralCover, filename = paste0("Final_Simulations/AddingMoreSensitivityAnalyses_MoreInitConds/ExtraGraphs/Rounded_SI_Tabu",buffsize[k],"_dval_",altsed[q],"_a",paramsies$a[j],"d",paramsies$d[j],"_",inity,"Cinit_allherbivores_FinalCoralCover_ScalingFactor",j,"_5.15.2025.png"), bg = "transparent", height = 10, width = 10)
      
      #initial malg cover
      Rounded_BaseRun_newrgamma_allherbivores_InitialMalgCover <-  
        ggplot(aes(x = long, y = lat), data = worldmap) + 
        geom_polygon(aes(group = group), fill="#f9f9f9", colour = "grey65") +
        xlab("Longitude") + ylab("Latitude")+ ggtitle(paste("Rounded Initial Malg Cover, Scaling Factor",j," d = ", altsed[q], "buffer size = ", buffsize[k],"a =",paramsies$a[j],"d_med = ",paramsies$d[j],"Cinit = ",inity))+
        ...
        scale_x_continuous(limits = c(180 - 3, 180 + 1), #c(180 - 3, 180 + 2.5) #c(180 - 3, 180)
                           ...
                           labels = c(160, 170, "180/-180", -170)) +
        coord_equal() +  theme_bw()
      ggsave(Rounded_BaseRun_newrgamma_allherbivores_InitialMalgCover, filename = paste0("Final_Simulations/AddingMoreSensitivityAnalyses_MoreInitConds/ExtraGraphs/Rounded_SI_Tabu",buffsize[k],"_dval_",altsed[q],"_a",paramsies$a[j],"d",paramsies$d[j],"_",inity,"Cinit_allherbivores_InitialMalgCover_ScalingFactor",j,"_5.15.2025.png"), bg = "transparent", height = 10, width = 10)
      
      #final malg cover
      Rounded_BaseRun_newrgamma_allherbivores_FinalMalgCover <-  
        ggplot(aes(x = long, y = lat), data = worldmap) + 
        geom_polygon(aes(group = group), fill="#f9f9f9", colour = "grey65") +
        xlab("Longitude") + ylab("Latitude")+ ggtitle(paste("Rounded Final Malg Cover, Scaling Factor",j," d = ", altsed[q], "buffer size = ", buffsize[k],"a =",paramsies$a[j],"d_med = ",paramsies$d[j],"Cinit = ",inity))+
        ...
        scale_x_continuous(limits = c(180 - 3, 180 + 1), #c(180 - 3, 180 + 2.5) #c(180 - 3, 180)
                           ...
                           labels = c(160, 170, "180/-180", -170)) +
        coord_equal() +  theme_bw()
      ggsave(Rounded_BaseRun_newrgamma_allherbivores_FinalMalgCover, filename = paste0("Final_Simulations/AddingMoreSensitivityAnalyses_MoreInitConds/ExtraGraphs/Rounded_SI_Tabu",buffsize[k],"_dval_",altsed[q],"_a",paramsies$a[j],"d",paramsies$d[j],"_",inity,"Cinit_allherbivores_FinalMalgCover_ScalingFactor",j,"_5.15.2025.png"), bg = "transparent", height = 10, width = 10)
...
#4.16.2025 - Plot the different initial condition scenarios
highcoral_varying <- data.frame(reef = seq(1,length(sitevector),1), C_init = NA, M_init = NA)

malg_placeholder <- (0.1/median(benthicfish_masterdataset$malg_cover))*(benthicfish_masterdataset$malg_cover)
malg_placeholder[which(malg_placeholder>0.99)] <- 0.98
coral_placeholder <- (0.8/median(benthicfish_masterdataset$coral_cover))*(benthicfish_masterdataset$coral_cover)
coral_placeholder[which(coral_placeholder>0.99)] <- 0.98
malg_placeholder[which(malg_placeholder+coral_placeholder > 1)] <- 0.01 #after this is done, none of the summed values are > 1

highcoral_varying$M_init <- malg_placeholder
highcoral_varying$C_init <- coral_placeholder
malg_placeholder <- coral_placeholder <- NA

highmalg_varying <- data.frame(reef = seq(1,length(sitevector),1), C_init = NA, M_init = NA)

malg_placeholder <- (0.8/median(benthicfish_masterdataset$malg_cover))*(benthicfish_masterdataset$malg_cover)
malg_placeholder[which(malg_placeholder>0.99)] <- 0.98
coral_placeholder <- (0.1/median(benthicfish_masterdataset$coral_cover))*(benthicfish_masterdataset$coral_cover)
coral_placeholder[which(coral_placeholder>0.99)] <- 0.98
coral_placeholder[which(malg_placeholder+coral_placeholder > 1)] <- 0.01 #after this is done, none of the summed values are > 1

highmalg_varying$M_init <- malg_placeholder
highmalg_varying$C_init <- coral_placeholder
malg_placeholder <- coral_placeholder <- NA


#coral cover
plot(x = seq(1,length(sitevector),1),y = benthicfish_masterdataset$coral_cover, pch = 20, ylim = c(0,1), xlab = "Site", ylab = "Coral Cover")
abline(h = 0.1, col = "darkgoldenrod")
points(x = seq(1,length(sitevector),1), y = highmalg_varying$C_init, pch = 20, col = "darkgoldenrod")
abline(h = 0.8, col = "deeppink2")
points(x = seq(1,length(sitevector),1), y = highcoral_varying$C_init, pch = 20, col = "deeppink2")

#malg cover
plot(x = seq(1,length(sitevector),1),y = benthicfish_masterdataset$malg_cover, pch = 20, ylim = c(0,1), xlab = "Site", ylab = "Macroalgae Cover")
abline(h = 0.1, col = "deeppink2")
points(x = seq(1,length(sitevector),1), y = highmalg_varying$M_init, pch = 20, col = "darkgoldenrod")
abline(h = 0.8, col = "darkgoldenrod")
points(x = seq(1,length(sitevector),1), y = highcoral_varying$M_init, pch = 20, col = "deeppink2")
