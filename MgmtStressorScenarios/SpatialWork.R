...
save(benthicfish_masterdataset_sed, file = "~/GitHub/PhDThesisProjects/Fiji_StabilityConnectivitySimulation/MgmtStressorScenarios/benthicfish_masterdataset_sedimentlvls_5.10.2022.RData")
...
#run this and can skip the next section
load("~/GitHub/PhDThesisProjects/Fiji_StabilityConnectivitySimulation/MgmtStressorScenarios/benthicfish_masterdataset_sed_updatedmgmtlevels_5.10.2022.RData") #old: 4.25.2022
...
save(benthicfish_masterdataset_sed, file = "~/GitHub/PhDThesisProjects/Fiji_StabilityConnectivitySimulation/MgmtStressorScenarios/benthicfish_masterdataset_sed_updatedmgmtlevels_5.10.2022.RData") #old: 4.25.2022
...
tabureefs_latlong_shp_buffer = st_buffer(tabureefs_latlong_shp, 0.02) #0.05
...
#save(benthicfish_masterdataset_sed, file = "~/GitHub/PhDThesisProjects/Fiji_StabilityConnectivitySimulation/MgmtStressorScenarios/benthicfish_masterdataset_sed_tabubuffer0.05_5.26.2022.RData")
...
save(benthicfish_masterdataset_sed, file = "~/GitHub/PhDThesisProjects/Fiji_StabilityConnectivitySimulation/MgmtStressorScenarios/benthicfish_masterdataset_sed_tabubuffer0.02_qoliqolionly_6.30.2022.RData")
...
benthicfish_latlong_shp <- st_as_sf(x = benthicfish_masterdataset_latlong, 
                                    coords = c("longitude", "latitude"),
                                    crs = st_crs(4326)) #just used the EPSG code directly
#old: crs = "+init=epsg:4326 +proj=merc +datum=WGS84");older: "+proj=merc +datum=WGS84"
#^ this one has reasonable lat/long's but st_crs()'s need to be the same, so need to adjust it
#when i add '+init=epsg:3832' it still says epsg is NA but when i do st_transform it doesn't change from NA and st_intersects doesn't throw an error but still shows no intersections

#make this into a shape file so i can play with it in qgis
#st_write(benthicfish_latlong_shp, "~/GitHub/PhDThesisProjects/Fiji_StabilityConnectivitySimulation/benthicfish_masterdataset_latlong.shp")
#st_write(benthicfish_latlong_shp_new, "~/GitHub/PhDThesisProjects/Fiji_StabilityConnectivitySimulation/benthicfish_masterdataset_latlong_5.10.2022.shp")
...
#this one (intersectionz) runs but a bunch of them don't intersect, seems like the lau reefs don't show up in the local reef pressures dataset? 
intersectionz <- st_intersects(x=benthicfish_latlong_shp, y=melanesia_allreefsWGS84) #melanesia_allreefsWGS84 or allreefs_WGS84 both give functionally the same thing (the numbers are the rows of y)
#prepared = TRUE maybe not needed? #sparse = FALSE gave a boolean matrix which was annoying

#this plot isn't that informative
plot(melanesia_allreefsWGS84$geometry)
#points(benthicfish_masterdataset$long.recenter, benthicfish_masterdataset$latitude, pch = 19, col = "blue")
#points(benthicfish_latlong_shp$geometry, pch = 19, col = "blue")
plot(benthicfish_latlong_shp$geometry, pch = 19, col = "blue", cex = 0.1, add = TRUE)

#need to match the sediment levels now, if no intersection just give it an NA for now (will match it to the median? later)
#some of them are really close to polygons and some of them are very far away, but i think in general just giving it a natural coral mortality of 0.24 makes the most sense

#the elements of intersectionz give the row of the melanesia dataset (i.e. the polygon) associated with each reef in the benthicfish dataset
benthicfish_masterdataset$sediment_orig <- NA
for(i in 1:(dim(benthicfish_masterdataset)[1])){
  if(length(intersectionz[[i]]) > 0){
    benthicfish_masterdataset$sediment_orig[i] <- melanesia_allreefsWGS84$sedimnt[intersectionz[[i]]]  
  }
}

hist(benthicfish_masterdataset$sediment_orig) #not normally distributed, focus on the median

#want these values to be centred at 0.24, set median = 0.24
goalmed <- 0.24
#remove the NAs
sediment_origvals <- benthicfish_masterdataset$sediment_orig[which(!(is.na(benthicfish_masterdataset$sediment_orig)))]
#find value to scale the dataset so the median value is 0.24
sediment_scale <- goalmed/median(sediment_origvals)
#scale the sediment values
benthicfish_masterdataset$sediment_alt <- sediment_scale*benthicfish_masterdataset$sediment_orig
#replace all of the NAs with 0.24
benthicfish_masterdataset$sediment_alt[is.na(benthicfish_masterdataset$sediment_alt)] <- goalmed
#sediment_alt is now the new mortality level for the reefs

hist(benthicfish_masterdataset$sediment_alt) #looks roughly the same 

#UPDATE/EDIT (4.24.2022): i think the 0's are causing errors, need to replace them with some other value. maybe 0.01 as that's quite a bit lower than the rest of the values?
benthicfish_masterdataset$sediment_altalt <- benthicfish_masterdataset$sediment_alt
benthicfish_masterdataset$sediment_altalt[which(benthicfish_masterdataset$sediment_altalt == 0)] <- 0.01

#save in a new dataframe
benthicfish_masterdataset_sed <- benthicfish_masterdataset
save(benthicfish_masterdataset_sed, file = "~/GitHub/PhDThesisProjects/Fiji_StabilityConnectivitySimulation/MgmtStressorScenarios/benthicfish_masterdataset_sedimentlvls_4.24.2022.RData")
#save(benthicfish_masterdataset_sed, file = "~/GitHub/PhDThesisProjects/Fiji_StabilityConnectivitySimulation/MgmtStressorScenarios/benthicfish_masterdataset_sedimentlvls_4.20.2022.RData")
...
NatCoralMortality_wzeros <-  
  ...
  scale_x_continuous(limits = c(180 - 3, 180 + 1), #c(180 - 3, 180 + 2.5) #c(180 - 3, 180)
                     ...
                     labels = c(160, 170, "180/-180", -170)) +
  coord_equal() +  theme_bw()
ggsave(NatCoralMortality_wzeros, filename = paste0("~/GitHub/PhDThesisProjects/Fiji_StabilityConnectivitySimulation/MgmtStressorScenarios/BaseSedimentScenario4.2022/NatCoralMortality_wzeros.png"), bg = "transparent", height = 10, width = 10)
...
NatCoralMortality_nozeros <-  
  ...
  scale_x_continuous(limits = c(180 - 3, 180 + 1), #c(180 - 3, 180 + 2.5) #c(180 - 3, 180)
                     ...
                     labels = c(160, 170, "180/-180", -170)) +
  coord_equal() +  theme_bw()
ggsave(NatCoralMortality_nozeros, filename = paste0("~/GitHub/PhDThesisProjects/Fiji_StabilityConnectivitySimulation/MgmtStressorScenarios/BaseSedimentScenario4.2022/NatCoralMortality_nozeros.png"), bg = "transparent", height = 10, width = 10)
